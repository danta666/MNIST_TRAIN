...
myGraph = tf.Graph()  
with myGraph.as_default():  # 重构相同的网络,这里和mnist.py中的网络一致
...
with tf.Session(graph=myGraph) as sess:
    ...
    saver.restore(sess,'这里写mnist.py中保存模板的路径') # restore参数
    
    array = [] 
    for i in range(10):
     #请把你需要识别的手写数字图片按照其序号排序放在指定的文件夹中，然后修改如下代码中的路径
     array.append(imageprepare('/home/Desktop/digits/'+str(i)+'.png'))#read the digits images

    
    for i in range(10):
     prediction = tf.argmax(y_conv, 1) # 预测
     prediction = prediction.eval(feed_dict={x_raw:[array[i]],keep_prob:1.0},session=sess)
